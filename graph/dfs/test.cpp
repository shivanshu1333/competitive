...
using namespace std;
    ...
    void dfs(vector<vector<int>>& A, int i, int j, int &ans, int cs){
        // if(i!=A.size()-1 && j==A[0].size())
        if(i==A.size()-1){
            // vis[i][j]=1;
            cs += A[i][j];
            ans = min(ans, cs);
